...
module.exports = class ESLintReporter extends Reporter {
  ...
  launch (done) {
    ...
    this.processFiles(this.options.src, this.options);
  ...
  processFiles (glob) {
    this.openReporter();
    const result = this.linter.executeOnFiles(Array.isArray(glob) ? glob : [glob]);
    let severity;

    result.results.forEach((result) => {
      this.openFileIssues(result.filePath, null, /^(\s+)?\n$/gm);
